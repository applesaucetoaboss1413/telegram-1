...
const getUser = (id) => {
    ...
    if (!strId || !/^\d+$/.test(strId)) {
        logger.warn('Invalid user ID passed to getUser', { id });
        return { id: strId, points: 0, created_at: Date.now(), is_premium: 0, referred_by: null, has_purchased: 0 };
    }

    let user = db.prepare('SELECT * FROM users WHERE id = ?').get(strId);

    // Admin override from env only (no hardcoded fallback IDs)
    const adminIds = (process.env.ADMIN_IDS || '').split(',').map(s => s.trim()).filter(Boolean);
    if (adminIds.includes(strId)) {
        if (!user) {
            const now = Date.now();
            db.prepare('INSERT INTO users (id, points, created_at) VALUES (?, ?, ?)').run(strId, 10000, now);
            logger.info(`admin user ${strId} created with balance=10000`);
            return { id: strId, points: 10000, created_at: now, is_premium: 0, referred_by: null, has_purchased: 0 };
        } else if (user.points < 10000) {
            db.prepare('UPDATE users SET points = 10000 WHERE id = ?').run(strId);
            logger.info(`admin user ${strId} balance updated to 10000`);
            user.points = 10000;
        }
    }
    ...
    if (!user) {
        const now = Date.now();
        db.prepare('INSERT INTO users (id, points, created_at) VALUES (?, 0, ?)').run(strId, now);
        return { id: strId, points: 0, created_at: now, is_premium: 0, referred_by: null, has_purchased: 0 };
    }

    // Update last activity
    db.prepare('UPDATE users SET last_activity = ? WHERE id = ?').run(Date.now(), strId);
