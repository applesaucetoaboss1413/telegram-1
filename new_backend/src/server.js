...
// ============ INPUT VALIDATION FOR HTTP ============

// Validate and sanitize string input
function sanitizeString(input, maxLength = 200) {
    if (typeof input !== 'string') return '';
    return input.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, '').trim().substring(0, maxLength);
}
...
// Simple IP-based rate limiter for HTTP endpoints
const httpRateLimits = new Map();
function httpRateLimit(identifier, max = 30, windowMs = 60000) {
    const now = Date.now();
    const key = String(identifier);
    if (!httpRateLimits.has(key)) {
        httpRateLimits.set(key, []);
    }
    const timestamps = httpRateLimits.get(key).filter(ts => now - ts < windowMs);
    httpRateLimits.set(key, timestamps);
...
setInterval(() => {
    const now = Date.now();
    for (const [key, timestamps] of httpRateLimits.entries()) {
        const fresh = timestamps.filter(ts => now - ts < 300000);
        if (fresh.length === 0) httpRateLimits.delete(key);
        else httpRateLimits.set(key, fresh);
    }
}, 300000);
...
// Admin Endpoint - with proper validation
app.post('/admin/grant', (req, res) => {
    const { secret, userId, amount } = req.body;

    // Rate limit admin endpoint by IP
    const clientIp = req.ip || req.connection.remoteAddress || 'unknown';
    if (!httpRateLimit(`admin:${clientIp}`, 10, 60000)) {
        logger.warn('Admin grant rate limited', { ip: clientIp });
    ...
    if (!secret || secret !== process.env.ADMIN_SECRET) {
        logger.warn('Invalid admin secret provided', { userId, ip: clientIp });
...
// Mini App: Get credits - with validation
app.get('/api/miniapp/credits', (req, res) => {
    const { userId } = req.query;
    if (!userId || !isValidUserId(userId)) return res.status(400).json({ error: 'Valid userId required' });

    try {
        const credits = getCredits({ telegramUserId: sanitizeString(userId, 20) });
        res.json({ credits });
    } catch (e) {
...
// Mini App: Get recent creations - with validation
...
// Mini App: Process service - with validation
app.post('/api/miniapp/process', validateCurrency, async (req, res) => {
    const { userId, service, files, inputs } = req.body;

    if (!userId || !isValidUserId(userId) || !service) {
        return res.status(400).json({ error: 'Valid userId and service required' });
    }

    // Validate service name against known services
    const validServices = ['faceswap', 'avatar', 'img2vid', 'enhance', 'bgremove'];
    if (!validServices.includes(service)) {
        return res.status(400).json({ error: 'Invalid service' });
    }

    // Rate limit per user
    if (!httpRateLimit(`process:${userId}`, 10, 60000)) {
...
// Mini App: Create checkout - with validation
app.post('/api/miniapp/checkout', validateCurrency, async (req, res) => {
    const { userId, packType, currency } = req.body;

    if (!userId || !isValidUserId(userId) || !packType) {
        return res.status(400).json({ error: 'Valid userId and packType required' });
    }
    ...
    // Rate limit checkouts per user
    if (!httpRateLimit(`checkout:${userId}`, 5, 60000)) {
