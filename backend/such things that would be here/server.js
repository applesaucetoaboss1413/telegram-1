require('dotenv').config({ path: require('path').join(__dirname, '../.env') }); // Adjusted path for .env since we move this file later? No, assuming it runs from backend root.
// Wait, if I write to 'such things...', the __dirname will be deeper.
// But I will move it to 'backend/server.js' before running.
// So I should keep the content as if it is in 'backend/server.js'.
// Original: require('dotenv').config({ path: require('path').join(__dirname, '.env') });
// This is correct if file is in backend/server.js.
