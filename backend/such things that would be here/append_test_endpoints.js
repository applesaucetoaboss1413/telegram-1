...
console.log('Server.js content (first 500 chars):', code.substring(0, 500));

if (/app\.get\('\/healthz'\)/.test(code)) {
  console.log('healthz already exists');
  process.exit(0);
}

const anchor = "\n// --- Configuration & Setup ---";
console.log('Anchor string:', anchor);
const idx = code.indexOf(anchor);
console.log('Anchor index:', idx);
if (idx === -1) {
  console.error('root anchor not found');
  process.exit(1);
}
...
const updated = code.replace(anchor, addition + anchor);
fs.writeFileSync(target, updated.replace(/\n/g, '\r\n'));
console.log('Appended test endpoints above root handler');
